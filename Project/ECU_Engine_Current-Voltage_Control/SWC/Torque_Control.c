 ...
 * @details hàm đọc dữ liệu cảm biến và tính toán mo men xoắn động cơ
 * @param   void
 * @return  void
 */

void TorqueControl_Update(void) {
    float throttle_input = 0.0f;    // bàn đạp ga
    float current_speed = 0.0f;     // tốc độ hiện tại
    float load_weight = 0.0f;       // tải trọng
    float actual_torque = 0.0f;     // momen xoắn thực tế
    float desired_torque = 0.0f;    // momen xoắn mong muốn
